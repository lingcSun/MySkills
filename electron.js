...
async function startServer() {
  ...
  try {
    ...
    function saveConfig(config) {
      if (!config.excluded_skills) config.excluded_skills = []
      if (!config.skill_remotes) config.skill_remotes = {}
      if (!config.skill_sources) {
        config.skill_sources = {
          ClaudeCode: config.skill_paths || [],
          Cursor: [],
        ...
        for (const entry of entries) {
          ...
          if (hasGit) {
            try {
              ...
              currentVersion = tags.latest || null
            } catch (e) {}
          }
    ...
    app.post('/api/skills/exclude', (req, res) => {
      ...
      if (!config.excluded_skills) config.excluded_skills = []
      if (!config.excluded_skills.includes(name)) config.excluded_skills.push(name)
      saveConfig(config)
    ...
    app.post('/api/skills/include', (req, res) => {
      const { name } = req.body
      const config = loadConfig()
      if (config.excluded_skills) config.excluded_skills = config.excluded_skills.filter(n => n !== name)
      saveConfig(config)
      res.json({ success: true })
    })
    ...
    app.post('/api/skills/set-remote', async (req, res) => {
      ...
      try {
        ...
        if (skill && skill.has_git) {
          ...
          if (hasOrigin) await git.remote(['set-url', 'origin', repoUrl])
          else await git.remote(['add', 'origin', repoUrl])
        }
        res.json({ success: true })
      } catch (e) {
    ...
    app.post('/api/skills/pull', async (req, res) => {
      ...
      try {
        ...
        await git.fetch('origin')
        if (strategy === 'merge') await git.merge(['origin/main'])
        else await git.reset(['--hard', 'origin/main'])
...
console.log('electron.js loaded!')
